...
namespace StreamUP
{
    ...
    public partial class StreamUpLib
    {
        ...
        // Main API methods for product Settings and Main actions
        // ============================================================

        #region Entry Points

        /// <summary>
        /// Check if product is ready for the Main Action.
        ...
        /// <summary>
        /// Check if product is ready with detailed status.
        ...
        public ProductReadyStatus CheckProductReady()
        {
            ...
            LogInfo($"[EntryPoint] Checking if product is ready: {productNumber}");

            // Try to get cached config first
            var config = ProductConfigRegistry.GetProductConfig(productNumber);
            if (config == null)
            {
                LogDebug($"[EntryPoint] Product config not found for: {productNumber}");
                LogDebug($"[EntryPoint] The Settings action needs to run first to register the product.");
            ...
            {
                LogDebug($"[EntryPoint] Using cached validation for: {productName}");
            ...
            LogInfo($"[EntryPoint] Product ready check result: {status}");
            return status;
        }

        /// <summary>
        /// Prepare for opening settings.
        ...
        public bool IsSettingsReady(JObject config)
        {
            if (config == null)
            {
                LogError("[EntryPoint] Config is null in IsSettingsReady");
                return false;
            }
            ...
            LogInfo($"[EntryPoint] Checking if settings ready for: {productName}");
            ...
            if (!ValidateLibraryVersionV2(libraryVersion))
            {
                ...
                {
                    LogDebug($"[EntryPoint] Already prompted about library version");
                    return false;
                }
            ...
            LogInfo($"[EntryPoint] Settings ready for: {productName}");
            return true;
        }
        ...
        public bool IsSettingsReady(string configJson)
        {
            ...
            {
                LogError($"[EntryPoint] Failed to parse config JSON: {ex.Message}");
                return false;
            }
        }

        /// <summary>
        /// Open settings with automatic registration.
        ...
        public bool OpenProductSettings(JObject config)
        {
            if (config == null)
            {
                LogError("[EntryPoint] Config is null in OpenProductSettings");
                return false;
            }
        ...
        public void InvalidateProductValidation()
        {
            ...
            LogDebug($"[EntryPoint] Validation cache invalidated for: {_ProductIdentifier}");
        }
