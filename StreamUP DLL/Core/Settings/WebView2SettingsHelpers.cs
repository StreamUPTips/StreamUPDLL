...
namespace StreamUP
{
    public partial class StreamUpLib
    {
        ...
        /// <summary>
        /// Get product info as a typed class.
        /// Use this with the generated ProductInfo class from the Settings Builder.
        /// Product info comes from the config JSON registered when the Settings action runs.
        /// </summary>
        /// <typeparam name="T">The typed ProductInfo class (generated by Settings Builder)</typeparam>
        /// <returns>Instance of T with product info populated, or new instance with defaults if not available</returns>
        /// <example>
        /// <code>
        /// // Using generated typed class from Settings Builder:
        /// var info = sup.GetProductInfo&lt;ProductInfo&gt;();
        /// string sceneName = info.SceneName;  // IntelliSense + type safety!
        /// string productVersion = info.ProductVersion;
        /// </code>
        /// </example>
        public T GetProductInfo<T>() where T : class, new()
        {
            LogDebug($"Getting product info as {typeof(T).Name}");

            try
            {
                // Get registered config (registered when Settings action called OpenProductSettings)
                var config = ProductConfigRegistry.GetProductConfig(_ProductIdentifier);
                if (config == null)
                {
                    LogDebug("No config registered, returning default instance");
                    return new T();
                }

                var productInfo = config["productInfo"] as JObject;
                if (productInfo == null || !productInfo.HasValues)
                {
                    LogDebug("No productInfo found in config, returning default instance");
                    return new T();
                }

                // Use Replace for object creation to prevent merging with defaults
                var serializer = new JsonSerializer
                {
                    ObjectCreationHandling = ObjectCreationHandling.Replace
                };

                return productInfo.ToObject<T>(serializer) ?? new T();
            }
            catch (Exception ex)
            {
                LogError($"Failed to deserialize product info to {typeof(T).Name}: {ex.Message}");
                return new T();
            }
        }
